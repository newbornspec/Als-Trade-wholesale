import { Link, NavLink } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { whatsappLink } from '../config/whatsapp';
import './Footer.css';
import logo from '../assets/logo.png';

export default function Footer() {
  const { user, logout } = useAuth();
  const navClass = ({ isActive }) => (isActive ? 'active' : '');

  return (
    <footer className="footer">
      <div className="footer-card">

        {/* ── Top row ── */}
        <div className="footer-top">
          <div className="footer-brand">
            <div className="footer-brand-row">
              <img src={logo} alt="A.L.S Trade" className="footer-logo-img" />
              <span className="footer-brand-sep" />
              <p className="footer-tagline">Worldwide import &amp; export of bulk IT hardware.</p>
            </div>
            <a href={whatsappLink()} target="_blank" rel="noreferrer" className="footer-wa">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              WhatsApp us
            </a>
          </div>

          <nav className="footer-nav">
            <NavLink to="/available-stock" className={navClass}>Available stock</NavLink>
            <NavLink to="/sold-stock"      className={navClass}>Sold stock</NavLink>
            <NavLink to="/how-it-works"    className={navClass}>How it works</NavLink>
            <NavLink to="/about-us"        className={navClass}>About us</NavLink>
            <NavLink to="/contact"         className={navClass}>Contact</NavLink>
          </nav>

          <div className="footer-contact">
            <a href="tel:02037471310">0203 747 1310</a>
            <a href="mailto:Sales@alswholesale.co.uk">Sales@alswholesale.co.uk</a>
          </div>
        </div>

        <div className="footer-divider" />

        {/* ── Bottom bar ── */}
        <div className="footer-bottom">
          <p className="footer-copy">
            © {new Date().getFullYear()} A.L.S Trade Ltd · Company No: 11269566
          </p>
          <p className="footer-address">
            Bizspace Business Park, Redfern Road · Birmingham B11 2AL · United Kingdom
          </p>
          <div className="footer-account">
            {user ? (
              <button type="button" onClick={logout} className="footer-link-btn">Log out</button>
            ) : (
              <Link to="/sign-in">Log in</Link>
            )}
          </div>
        </div>

      </div>
    </footer>
  );
}
